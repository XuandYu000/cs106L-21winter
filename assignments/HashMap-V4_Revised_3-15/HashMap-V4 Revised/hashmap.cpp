...
/*
Range constructor: 先用delegating constructor 将bucket_count和hash初始化
*/
template <typename K, typename M, typename H>
...
bool operator==(const HashMap<K, M, H>& lhs, const HashMap<K, M, H>& rhs) {
    ...
    for(auto& [key, mapped] : lhs) {
        if(!rhs.contains(key) || rhs.at(key) != mapped) return false;
    }
    return true;
}
...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const HashMap& rhs) : _size{rhs._size}, _hash_function{rhs._hash_function}, _buckets_array{rhs.bucket_count(), nullptr} {
...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(HashMap&& rhs) : _size{std::move(rhs._size)}, _hash_function{std::move(rhs._hash_function)}, _buckets_array{rhs.bucket_count(), nullptr} {
